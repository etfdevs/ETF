...
#define TRACEMAP_SIZE				512            // keeg default was 256, we want more granulity
...
void CG_GenerateTracemap( void ) {
	...
	if( !cg.mapcoordsValid ) {
		CG_Printf(BOX_PRINT_MODE_CHAT, "Need valid mapcoords in the worldspawn to be able to generate a tracemap.\n" );
		return;
	}

	if( (cg.mapcoordsMaxs[0] - cg.mapcoordsMins[0]) != (cg.mapcoordsMins[1] - cg.mapcoordsMaxs[1]) ) {
		CG_Printf( BOX_PRINT_MODE_CHAT,"Mapcoords need to be square.\n" );
		return;
	}

	// Topdown tracing
	CG_Printf(BOX_PRINT_MODE_CHAT, "Generating level heightmap and level mask...\n" );
	...
	// Sky tracing
	CG_Printf( BOX_PRINT_MODE_CHAT,"Generating sky heightmap and sky mask...\n" );
	...
	CG_Printf( BOX_PRINT_MODE_CHAT, "Generating sky groundmap...\n" );
...
//		getskytime += trap_Milliseconds() - msec;
	...
	i = Q_ftol(( point[0] - tracemap.world_mins[0] ) * one_over_mapgrid_factor[0]);
	j = Q_ftol(( point[1] - tracemap.world_mins[1] ) * one_over_mapgrid_factor[1]);

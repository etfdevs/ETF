...
#define	FLAME_START_MAX_SIZE	100.0	// when the flame is spawned, it should endevour to reach this size 140
#define	FLAME_START_MAX_SIZE_RAND	60.0
#define	FLAME_MAX_SIZE			100.0	// flame sprites cannot be larger than this  keeg default is 200
#define	FLAME_MIN_MAXSIZE		40.0	// don't ever let the sizeMax go less than this
#define	FLAME_START_SPEED		1400.0//1200.0	// speed of flame as it leaves the nozzle keeg
...
===============
*/
void CG_FireFlameChunks( centity_t *cent, vec3_t origin, vec3_t angles, qboolean firing )
{
...
void CG_AddFlameSpriteToScene( flameChunk_t *f, float lifeFrac, float alpha )
{
	...
	pPolyBuffer = CG_PB_FindFreePolyBuffer( cgs.media.flameShaders[frameNum], 4, 6 );
...
*/
	...
	for (i=0; i<NUM_FLAME_SPRITES; i++) {
		...
		cgs.media.flameShaders[i] = trap_R_RegisterShader( filename );
	}
...
===============
Keeg:  go through and play with the sounds, all changed for now...
*/
	...
	while (f) {
		...
		if (centFlameInfo[f->ownerCent].lastSoundUpdate != cg.time) {
			...
			if (centFlameStatus[f->ownerCent].blowVolume*255.0 > MIN_BLOW_VOLUME) {
				trap_S_AddLoopingSound( 0, f->org, vec3_origin, cgs.media.sfx_flamethrower_fire, (int)(255.0*centFlameStatus[f->ownerCent].blowVolume), 0 ); // JPW NERVE
			} else {
				trap_S_AddLoopingSound( 0, f->org, vec3_origin, cgs.media.sfx_flamethrower_fire, MIN_BLOW_VOLUME, 0 ); // JPW NERVE
			}

			if (centFlameStatus[f->ownerCent].streamVolume) {
					trap_S_AddLoopingSound( 0, f->org, vec3_origin, cgs.media.sfx_flamethrower_fire, (int)(255.0*centFlameStatus[f->ownerCent].streamVolume), 0 ); // JPW NERVE
			}
		...
		for (trav=f; trav; trav=trav->nextFlameChunk) {
			...
			if (trav->blueLife+100 < (cg.time - trav->timeStart)) {
				trap_S_AddLoopingSound( 0, trav->org, vec3_origin, cgs.media.sfx_flamethrower_fire, (int)(255.0*(0.2 * (trav->size / FLAME_MAX_SIZE))), 0 );
			}
		}

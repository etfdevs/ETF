...
__inline int64_t cJSON_ToLongIntegerRaw( cJSON *item ) {
	return item->valuelong;
}
...
void cJSON_SetIntegerValue( cJSON *item, int integer ) {
	...
	item->valuelong = (int64_t)integer;
}

void cJSON_SetLongIntegerValue( cJSON *item, int64_t integer ) {
	if ( item->type == cJSON_Array || item->type == cJSON_Object ) {
		return;
	}
	item->type = cJSON_Number;
	...
	item->valueint = (int)integer;
	item->valuedouble = (double)integer;
	item->valuelong = integer;
}

...
#ifdef API_Q3
	SystemCall( CG_R_ADDLIGHTTOSCENE, org, PASSFLOAT(radius), PASSFLOAT(r), PASSFLOAT(g), PASSFLOAT(b) );
#endif
#ifdef API_ET
...
int trap_R_LerpTag( orientation_t *tag, const refEntity_t *refent, const char *tagName, int startIndex ) {
	...
	return SystemCall( CG_R_LERPTAG, tag, refent->hModel, refent->oldframe, refent->frame, PASSFLOAT(backLerp), tagName );
#endif
#ifdef API_ET
...
#endif
	SystemCall( CG_S_ADDLOOPINGSOUND, entityNum, origin, velocity, sfx );
#endif
#ifdef API_ET
...
void trap_S_AddRealLoopingSound( int entityNum, const vec3_t origin, const vec3_t velocity, sfxHandle_t sfx, int range, int volume, int soundTime ) {
	...
	cgs.registeredSoundCounts[(int) sfx]++;
#endif
	SystemCall( CG_S_ADDREALLOOPINGSOUND, entityNum, origin, velocity, sfx );
#endif
#ifdef API_ET
...
#ifdef API_Q3
	SystemCall( CG_S_STARTBACKGROUNDTRACK, intro, loop );
#endif
#ifdef API_ET
...
#ifdef API_Q3
	SystemCall( CG_SETUSERCMDVALUE, stateValue, PASSFLOAT(sensitivityScale) );
#endif
#ifdef API_ET

...
#ifdef API_Q3
	SystemCall( UI_R_ADDLIGHTTOSCENE, org, PASSFLOAT(radius), PASSFLOAT(r), PASSFLOAT(g), PASSFLOAT(b) );
#endif
#ifdef API_ET
...
#ifdef API_Q3
	return SystemCall( UI_CM_LERPTAG, tag, refent->hModel, refent->oldframe, refent->frame, PASSFLOAT(1.0 - refent->backlerp), tagName );
#endif
#ifdef API_ET
...
#ifdef API_Q3
	SystemCall( UI_S_STARTBACKGROUNDTRACK, intro, loop );
#endif
#ifdef API_ET

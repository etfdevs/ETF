...
void trap_Cvar_LatchedVariableStringBuffer( const char *var_name, char *buffer, int bufsize ) {
	Q_syscall( UI_CVAR_LATCHEDVARIABLESTRINGBUFFER, var_name, buffer, bufsize );
}
...
void trap_AddCommand( const char *cmdName ) {
	Q_syscall( UI_ADDCOMMAND, cmdName );
}
...
int trap_FS_Delete(const char *filename) {
	return Q_syscall( UI_FS_DELETEFILE, filename);
}
...
void trap_R_RegisterFont(const char *fontName, int pointSize, fontInfo_t *font) {
	Q_syscall( UI_R_REGISTERFONT, fontName, pointSize, font );
}
...
//%		Q_syscall( UI_R_ADDLIGHTTOSCENE, org, PASSFLOAT(intensity), PASSFLOAT(r), PASSFLOAT(g), PASSFLOAT(b), overdraw );
//%	}
...
void trap_R_AddCoronaToScene( const vec3_t org, float r, float g, float b, float scale, int id, qboolean visible ) {
	Q_syscall( UI_R_ADDCORONATOSCENE, org, PASSFLOAT(r), PASSFLOAT(g), PASSFLOAT(b), PASSFLOAT(scale), id, visible  );
}
...
void trap_R_Add2dPolys( polyVert_t* verts, int numverts, qhandle_t hShader ) {
	Q_syscall( UI_R_DRAW2DPOLYS, verts, numverts, hShader );
}
...
void trap_R_DrawRotatedPic( float x, float y, float w, float h, float s1, float t1, float s2, float t2, qhandle_t hShader, float angle ) {
	Q_syscall( UI_R_DRAWROTATEDPIC, PASSFLOAT(x), PASSFLOAT(y), PASSFLOAT(w), PASSFLOAT(h), PASSFLOAT(s1), PASSFLOAT(t1), PASSFLOAT(s2), PASSFLOAT(t2), hShader, PASSFLOAT(angle) );
}
...
void trap_S_StartLocalSound( sfxHandle_t sfx, int channelNum ) {
	Q_syscall( UI_S_STARTLOCALSOUND, sfx, channelNum, 127 /* Gordon: default volume always for the moment*/ );
}

sfxHandle_t	trap_S_RegisterSound( const char *sample, qboolean compressed ) {
	int i = Q_syscall( UI_S_REGISTERSOUND, sample, qfalse /* compressed */ );
#ifdef _DEBUG
	if(i == 0) {
...
void	trap_S_FadeBackgroundTrack( float targetvol, int time, int num){	// yes, i know.  fadebackground coming in, fadestreaming going out.  will have to see where functionality leads...
	Q_syscall( UI_S_FADESTREAMINGSOUND, PASSFLOAT(targetvol), time, num);	// 'num' is '0' if it's music, '1' if it's "all streaming sounds"
}

void	trap_S_FadeAllSound( float targetvol, int time, qboolean stopsound ) {
	Q_syscall( UI_S_FADEALLSOUNDS, PASSFLOAT(targetvol), time, stopsound);
}
...
void trap_Key_KeysForBinding( const char* binding, int* key1, int* key2 ) {
	Q_syscall( UI_KEY_BINDINGTOKEYS, binding, key1, key2 );
}
...
int	trap_LAN_GetLocalServerCount( void ) {
	return Q_syscall( UI_LAN_GETLOCALSERVERCOUNT );
}

void trap_LAN_GetLocalServerAddressString( int n, char *buf, int buflen ) {
	Q_syscall( UI_LAN_GETLOCALSERVERADDRESSSTRING, n, buf, buflen );
}

int trap_LAN_GetGlobalServerCount( void ) {
	return Q_syscall( UI_LAN_GETGLOBALSERVERCOUNT );
}

void trap_LAN_GetGlobalServerAddressString( int n, char *buf, int buflen ) {
	Q_syscall( UI_LAN_GETGLOBALSERVERADDRESSSTRING, n, buf, buflen );
}
...
void trap_SetPbClStatus( int status ) {
	Q_syscall( UI_SET_PBCLSTATUS, status );
}
// DHM - Nerve

// TTimo: also for Sv
void trap_SetPbSvStatus( int status ) {
	Q_syscall( UI_SET_PBSVSTATUS, status );
}
...
void trap_GetCDKey( char *buf, int buflen ) {
	Q_syscall( UI_GET_CDKEY, buf, buflen );
}

void trap_SetCDKey( char *buf ) {
	Q_syscall( UI_SET_CDKEY, buf );
}
...
int trap_PC_RemoveAllGlobalDefines( void ) {
	return Q_syscall( UI_PC_REMOVE_ALL_GLOBAL_DEFINES );
}
...
int trap_PC_UnReadToken( int handle ) {
	return Q_syscall( UI_PC_UNREAD_TOKEN, handle );
}
...
qboolean trap_VerifyCDKey( const char *key, const char *chksum) {
	return Q_syscall( UI_VERIFY_CDKEY, key, chksum);
}

// NERVE - SMF
qboolean trap_GetLimboString( int index, char *buf ) {
	return Q_syscall( UI_CL_GETLIMBOSTRING, index, buf );
}
...
char* trap_TranslateString( const char *string ) {
	...
	Q_syscall( UI_CL_TRANSLATE_STRING, string, buf );
#else
...
void trap_CheckAutoUpdate( void ) {
	Q_syscall( UI_CHECKAUTOUPDATE );
}

void trap_GetAutoUpdate( void ) {
	Q_syscall( UI_GET_AUTOUPDATE );
}
// DHM - Nerve

void trap_openURL( const char *s ) {
	Q_syscall( UI_OPENURL, s );
}

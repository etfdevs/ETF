...
int		trap_FS_Write( const void *buffer, int len, fileHandle_t f ) {
	return Q_syscall( G_FS_WRITE, buffer, len, f );
}

int		trap_FS_Rename( const char *from, const char *to ) {
	return Q_syscall( G_FS_RENAME, from, to );
}
...
void trap_Cvar_LatchedVariableStringBuffer( const char *var_name, char *buffer, int bufsize ) {
	Q_syscall( G_CVAR_LATCHEDVARIABLESTRINGBUFFER, var_name, buffer, bufsize );
}
...
void trap_Trace( trace_t *results, const vec3_t start, const vec3_t mins, const vec3_t maxs, const vec3_t end, int passEntityNum, int contentmask ) {
	Q_syscall( G_TRACE, results, start, mins, maxs, end, passEntityNum, contentmask );
}
...
void trap_TraceNoEnts( trace_t *results, const vec3_t start, const vec3_t mins, const vec3_t maxs, const vec3_t end, int passEntityNum, int contentmask ) {
	Q_syscall( G_TRACE, results, start, mins, maxs, end, -2, contentmask );
}

void trap_TraceCapsule( trace_t *results, const vec3_t start, const vec3_t mins, const vec3_t maxs, const vec3_t end, int passEntityNum, int contentmask ) {
	Q_syscall( G_TRACECAPSULE, results, start, mins, maxs, end, passEntityNum, contentmask );
}
...
void trap_TraceCapsuleNoEnts( trace_t *results, const vec3_t start, const vec3_t mins, const vec3_t maxs, const vec3_t end, int passEntityNum, int contentmask ) {
	Q_syscall( G_TRACECAPSULE, results, start, mins, maxs, end, -2, contentmask );
}
...
int trap_BotAllocateClient( int clientNum ) {
	return Q_syscall( G_BOT_ALLOCATE_CLIENT, clientNum );
}
...
int trap_GetSoundLength(sfxHandle_t sfxHandle) {
	return Q_syscall( G_GET_SOUND_LENGTH, sfxHandle );
}

sfxHandle_t	trap_RegisterSound( const char *sample, qboolean compressed ) {
	return Q_syscall( G_REGISTERSOUND, sample, compressed );
}
...
qboolean trap_GetTag( int clientNum, int tagFileNumber, char *tagName, orientation_t *or ) {
	return Q_syscall( G_GETTAG, clientNum, tagFileNumber, tagName, or );
}

qboolean trap_LoadTag( const char* filename ) {
	return Q_syscall( G_REGISTERTAG, filename );
}
...
int trap_PC_UnReadToken( int handle ) {
	return Q_syscall( BOTLIB_PC_UNREAD_TOKEN, handle );
}
...
//	Q_syscall( BOTLIB_AAS_SETCURRENTWORLD, index );
}
// done.
...
void trap_AAS_AreaCenter(int areanum, vec3_t center) {
	Q_syscall( BOTLIB_AAS_AREA_CENTER, areanum, center);
}

qboolean trap_AAS_AreaWaypoint(int areanum, vec3_t center) {
	return Q_syscall( BOTLIB_AAS_AREA_WAYPOINT, areanum, center);
}
...
int trap_AAS_AreaLadder(int areanum) {
	return Q_syscall( BOTLIB_AAS_AREA_LADDER, areanum );
}
...
void trap_AAS_RT_ShowRoute( vec3_t srcpos, int	srcnum, int destnum ) {
	Q_syscall( BOTLIB_AAS_RT_SHOWROUTE, srcpos, srcnum, destnum );
}
...
//	return Q_syscall( BOTLIB_AAS_RT_GETHIDEPOS, srcpos, srcnum, srcarea, destpos, destnum, destarea, returnPos );
//}

//int trap_AAS_FindAttackSpotWithinRange(int srcnum, int rangenum, int enemynum, float rangedist, int travelflags, float *outpos) {
//	return Q_syscall( BOTLIB_AAS_FINDATTACKSPOTWITHINRANGE, srcnum, rangenum, enemynum, PASSFLOAT(rangedist), travelflags, outpos );
//}
...
{
	return Q_syscall( BOTLIB_AAS_NEARESTHIDEAREA, srcnum, origin, areanum, enemynum, enemyorigin, enemyareanum, travelflags, PASSFLOAT(maxdist), distpos );
}

int trap_AAS_ListAreasInRange(vec3_t srcpos, int srcarea, float range, int travelflags, float **outareas, int maxareas) {
	return Q_syscall( BOTLIB_AAS_LISTAREASINRANGE, srcpos, srcarea, PASSFLOAT(range), travelflags, outareas, maxareas );
}

int trap_AAS_AvoidDangerArea(vec3_t srcpos, int srcarea, vec3_t dangerpos, int dangerarea, float range, int travelflags) {
	return Q_syscall( BOTLIB_AAS_AVOIDDANGERAREA, srcpos, srcarea, dangerpos, dangerarea, PASSFLOAT(range), travelflags );
}
...
{
	return Q_syscall( BOTLIB_AAS_RETREAT, dangerSpots, dangerSpotCount, srcpos, srcarea, dangerpos, dangerarea, PASSFLOAT(range), PASSFLOAT(dangerRange),travelflags );
}
...
void trap_AAS_SetAASBlockingEntity( vec3_t absmin, vec3_t absmax, int blocking ) {
	Q_syscall( BOTLIB_AAS_SETAASBLOCKINGENTITY, absmin, absmax, blocking );
}

void trap_AAS_RecordTeamDeathArea( vec3_t srcpos, int srcarea, int team, int teamCount, int travelflags ) {
	Q_syscall( BOTLIB_AAS_RECORDTEAMDEATHAREA, srcpos, srcarea, team, teamCount, travelflags );
}
// done.
...
void trap_EA_UseItem(int client, char *it) {
	Q_syscall( BOTLIB_EA_USE_ITEM, client, it );
}

void trap_EA_DropItem(int client, char *it) {
	Q_syscall( BOTLIB_EA_DROP_ITEM, client, it );
}

void trap_EA_UseInv(int client, char *inv) {
	Q_syscall( BOTLIB_EA_USE_INV, client, inv );
}

void trap_EA_DropInv(int client, char *inv) {
	Q_syscall( BOTLIB_EA_DROP_INV, client, inv );
}
...
void trap_EA_Reload(int client) {
	Q_syscall( BOTLIB_EA_RELOAD, client );
}
...
void trap_EA_Walk(int client) {
	Q_syscall( BOTLIB_EA_WALK, client );
}
...
void trap_EA_ResetInput(int client, void *init) {
	Q_syscall( BOTLIB_EA_RESET_INPUT, client, init );
}

void trap_EA_Prone(int client) {
	Q_syscall( BOTLIB_EA_PRONE, client );
}

int trap_BotLoadCharacter(char *charfile, int skill) {
	return Q_syscall( BOTLIB_AI_LOAD_CHARACTER, charfile, skill);
}
...
void trap_BotSetChatName(int chatstate, char *name) {
	Q_syscall( BOTLIB_AI_SET_CHAT_NAME, chatstate, name );
}
...
void trap_BotInitAvoidReach(int handle) {
	Q_syscall( BOTLIB_AI_INIT_AVOID_REACH, handle );
}
// Done.

...
int BotTeam(bot_state_t *bs) {
	...
	//
	if (atoi(Info_ValueForKey(info, "t")) == Q3F_TEAM_RED) return Q3F_TEAM_RED;
	else if (atoi(Info_ValueForKey(info, "t")) == Q3F_TEAM_BLUE) return Q3F_TEAM_BLUE;
...
int BotOppositeTeam(bot_state_t *bs) {
	switch(BotTeam(bs)) {
		case Q3F_TEAM_RED: return Q3F_TEAM_BLUE;
		case Q3F_TEAM_BLUE: return Q3F_TEAM_RED;
		default: return Q3F_TEAM_FREE;
	}
}
...
void BotCTFSeekGoals(bot_state_t *bs) {
	...
	if (BotCTFCarryingFlag(bs)) {
		...
		if (bs->ltgtype != LTG_RUSHBASE) {
			...
			//
			switch(BotTeam(bs)) {
				case Q3F_TEAM_RED: VectorSubtract(bs->origin, ctf_blueflag.origin, dir); break;
				case Q3F_TEAM_BLUE: VectorSubtract(bs->origin, ctf_redflag.origin, dir); break;
			...
			if (BotTeam(bs) == Q3F_TEAM_RED) flagstatus = bs->redflagstatus;
...
/*		if (!EntityCarriesFlag(&entinfo)) {
	...
	//
	if (BotTeam(bs) == Q3F_TEAM_RED) flagstatus = bs->redflagstatus * 2 + bs->blueflagstatus;
	...
	if (rnd < l1 && ctf_redflag.areanum && ctf_blueflag.areanum) {
		...
		bs->ordered = qfalse;
		//
		if (BotTeam(bs) == Q3F_TEAM_RED) memcpy(&bs->teamgoal, &ctf_redflag, sizeof(bot_goal_t));
...
/*	if (gametype == GT_CTF
		...
		if (BotTeam(bs) == Q3F_TEAM_RED) context |= CONTEXT_CTFREDTEAM;
...
qboolean BotIsObserver(bot_state_t *bs) {
	...
	if (atoi(Info_ValueForKey(buf, "t")) == Q3F_TEAM_SPECTATOR) return qtrue;
	return qfalse;
}

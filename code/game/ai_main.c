...
/*	if (gametype == GT_CTF) {
		if (BotCTFCarryingFlag(bs)) {
			if (BotTeam(bs) == Q3F_TEAM_RED) strcpy(flagstatus, S_COLOR_RED"F ");
...
void BotTeamplayReport(void) {
	...
	for (i = 0; i < maxclients && i < MAX_CLIENTS; i++) {
		...
		//skip spectators
		if (atoi(Info_ValueForKey(buf, "t")) == Q3F_TEAM_RED) {
	...
	for (i = 0; i < maxclients && i < MAX_CLIENTS; i++) {
		...
		if (!strlen(buf) || !strlen(Info_ValueForKey(buf, "n"))) continue;
		//skip spectators
		if (atoi(Info_ValueForKey(buf, "t")) == Q3F_TEAM_BLUE) {
			BotReportStatus(botstates[i]);
		}
	}
}
...
void BotInputToUserCommand(bot_input_t *bi, usercmd_t *ucmd, int delta_angles[3], int time) {
	...
	if (bi->actionflags & ACTION_WAVENO)		ucmd->buttons |= BUTTON_WAVENO;
	if (bi->actionflags & ACTION_WAVEYES)		ucmd->buttons |= BUTTON_WAVEYES;
	if (bi->actionflags & ACTION_BECON)			ucmd->buttons |= BUTTON_BECON;
	if (bi->actionflags & ACTION_STOP)			ucmd->buttons |= BUTTON_STOP;
	if (bi->actionflags & ACTION_POINT)			ucmd->buttons |= BUTTON_POINT;
	if (bi->actionflags & ACTION_LOOK)			ucmd->buttons |= BUTTON_LOOK;
	//

	...
	for ( n = 0; n < g_numBots ; n++ ) {
		...
		if (i >= g_maxclients.integer) {
			...
			if (num <= 0) {
				...
				if (team == Q3F_TEAM_RED) teamstr = "red";
				else if (team == Q3F_TEAM_BLUE) teamstr = "blue";
				else teamstr = "";
...
//	if (g_gametype.integer >= GT_TEAM) {
		...
		humanplayers = G_CountHumanPlayers( Q3F_TEAM_RED );
		botplayers = G_CountBotPlayers(	Q3F_TEAM_RED );
		//
		if (humanplayers + botplayers < minplayers) {
			G_AddRandomBot( Q3F_TEAM_RED );
		} else if (humanplayers + botplayers > minplayers && botplayers) {
			G_RemoveRandomBot( Q3F_TEAM_RED );
		}
		//
		humanplayers = G_CountHumanPlayers( Q3F_TEAM_BLUE );
		botplayers = G_CountBotPlayers( Q3F_TEAM_BLUE );
		//
		if (humanplayers + botplayers < minplayers) {
			G_AddRandomBot( Q3F_TEAM_BLUE );
		} else if (humanplayers + botplayers > minplayers && botplayers) {
			G_RemoveRandomBot( Q3F_TEAM_BLUE );
		}
//	}
/*	else if (g_gametype.integer == GT_TOURNAMENT ) {
		...
		//
		if (humanplayers + botplayers < minplayers) {
			G_AddRandomBot( Q3F_TEAM_FREE );
		} else if (humanplayers + botplayers > minplayers && botplayers) {
			// try to remove spectators first
			if (!G_RemoveRandomBot( Q3F_TEAM_SPECTATOR )) {
		...
		humanplayers = G_CountHumanPlayers( Q3F_TEAM_FREE );
		botplayers = G_CountBotPlayers( Q3F_TEAM_FREE );
		//
		if (humanplayers + botplayers < minplayers) {
			G_AddRandomBot( Q3F_TEAM_FREE );
		} else if (humanplayers + botplayers > minplayers && botplayers) {
			G_RemoveRandomBot( Q3F_TEAM_FREE );
		}
	}*/
}
...
//	char			*headmodel;
	...
	if( !team || !*team ) {
//		if( g_gametype.integer >= GT_TEAM ) {
			if( PickTeam(clientNum) == Q3F_TEAM_RED) {
				team = "red";
			}
			else {
				team = "blue";
...
/*		else {
	...
	bot->inuse = qtrue;
	// Falcon : start : set bot classes
/*	if(Q_stricmp("Recon", name) == 0)
		bot->botclass = Q3F_CLASS_RECON;
	else if(Q_stricmp("Sniper", name) == 0)
		bot->botclass = Q3F_CLASS_SNIPER;
	else if(Q_stricmp("Soldier", name) == 0)
		bot->botclass = Q3F_CLASS_SOLDIER;
	else if(Q_stricmp("Grenadier", name) == 0)
		bot->botclass = Q3F_CLASS_GRENADIER;
	else if(Q_stricmp("Paramedic", name) == 0)
		bot->botclass = Q3F_CLASS_PARAMEDIC;
	else if(Q_stricmp("Minigunner", name) == 0)
		bot->botclass = Q3F_CLASS_MINIGUNNER;
	else if(Q_stricmp("Flametrooper", name) == 0)
		bot->botclass = Q3F_CLASS_FLAMETROOPER;
	else if(Q_stricmp("Agent", name) == 0)
		bot->botclass = Q3F_CLASS_AGENT;
	else if(Q_stricmp("Engineer", name) == 0)
		bot->botclass = Q3F_CLASS_ENGINEER;
	else if(Q_stricmp("Civilian", name) == 0)
		bot->botclass = Q3F_CLASS_CIVILIAN;
	else
	{
		G_Printf( S_COLOR_RED "Error: %s is an invalid class - bot spawned as Civilian.\n", name );
		bot->botclass = Q3F_CLASS_CIVILIAN;
	}*/
	// Falcon : end

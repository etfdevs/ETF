...
q3f_keypairarray_t *G_Q3F_LoadMapInfo( char *mapname )
{
			...
			G_Q3F_MI_GetToken( "{" ) )
		{
			...
			while( G_Q3F_MI_GetToken( NULL ) )
			{
				...
				else if( !Q_stricmp( mi.token.string, /*{*/"}" ) )
				{
					...
					G_Q3F_KeyPairArrayAdd( kpa, keyname, Q3F_TYPE_STRING, 0, (uintptr_t) mi.token.string );
					if( !G_Q3F_MI_GetToken( ";" ) )
						break;
				}
			}
	...
	kpa = G_Q3F_KeyPairArrayCreate();
	G_Q3F_KeyPairArrayAdd( kpa, "mapinfo", Q3F_TYPE_STRING, 0, (uintptr_t) rawmpi );
				...
				*ptr4 = 0;
				for( index = 0; matchStrings[index]; index++ )
				{
					if( !Q_stricmp( matchStrings[index], keyname ) )
					{
						...
						G_Q3F_KeyPairArrayAdd( kpa, "minplayers", Q3F_TYPE_STRING, 0, (uintptr_t) keyname );
						Com_sprintf( keyname, sizeof(keyname), "%d", c2 );
						G_Q3F_KeyPairArrayAdd( kpa, "maxplayers", Q3F_TYPE_STRING, 0, (uintptr_t) keyname );
						ptr = (char *) -1;
						break;
					}
				}

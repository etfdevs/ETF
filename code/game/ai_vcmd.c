...
void BotVoiceChat_Defend(bot_state_t *bs, int client, int mode) {
	/*if (gametype == GT_CTF ) {
		//
		switch(BotTeam(bs)) {
			case Q3F_TEAM_RED: memcpy(&bs->teamgoal, &ctf_redflag, sizeof(bot_goal_t)); break;
			case Q3F_TEAM_BLUE: memcpy(&bs->teamgoal, &ctf_blueflag, sizeof(bot_goal_t)); break;
			default: return;
		}
	}
	else*/ {

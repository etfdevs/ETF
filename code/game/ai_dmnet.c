...
/*#ifdef CTF
	if (gametype == GT_CTF) {
		...
		if (bs->ltgtype == LTG_GETFLAG) {
			...
			//
			switch(BotTeam(bs)) {
				case Q3F_TEAM_RED: memcpy(goal, &ctf_blueflag, sizeof(bot_goal_t)); break;
				case Q3F_TEAM_BLUE: memcpy(goal, &ctf_redflag, sizeof(bot_goal_t)); break;
			...
			if (trap_BotTouchingGoal(bs->origin, goal)) {
				...
				switch(BotTeam(bs)) {
					case Q3F_TEAM_RED: bs->blueflagstatus = 1; break;
					case Q3F_TEAM_BLUE: bs->redflagstatus = 1; break;
				}
				bs->ltgtype = 0;
			}
		...
		if (bs->ltgtype == LTG_RUSHBASE && bs->rushbaseaway_time < FloatTime()) {
			switch(BotTeam(bs)) {
				case Q3F_TEAM_RED: memcpy(goal, &ctf_redflag, sizeof(bot_goal_t)); break;
				case Q3F_TEAM_BLUE: memcpy(goal, &ctf_blueflag, sizeof(bot_goal_t)); break;
				default: bs->ltgtype = 0; return qfalse;
			}

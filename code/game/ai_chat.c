...
int BotNumActivePlayers(void) {
	...
	for (i = 0; i < maxclients && i < MAX_CLIENTS; i++) {
		...
		//skip spectators
		if (atoi(Info_ValueForKey(buf, "t")) == Q3F_TEAM_SPECTATOR) continue;
		//
		num++;
	}
	return num;
...
//	if (gametype == GT_TOURNAMENT) return qfalse;
	...
	else
	{
		...
		//
		if (bs->enemydeathtype == MOD_AXE) {
			BotAI_BotInitialChat(bs, "kill_axe", name, NULL);
		}
